...
def main(model_path, model_type, dataset_str, random_seed, train_ratio):
    ...
    diagramme_non_calibré = reliability_diagram(test_labels, torch.argmax(test_logits, dim=1), torch.softmax(test_logits, dim=1)[:, 1].numpy())
    diagramme_ts = reliability_diagram(test_labels, torch.argmax(test_logits / temperature_ts, dim=1), torch.softmax(test_logits / temperature_ts, dim=1)[:, 1].numpy())
    diagramme_ec = reliability_diagram(test_labels, torch.argmax(test_logits / temperature_ec, dim=1), torch.softmax(test_logits / temperature_ec, dim=1)[:, 1].numpy())
