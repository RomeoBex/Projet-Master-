...
En comparant @fig-4 avec @fig-5 et @fig-6 on voit que pour ce modèle, les deux méthodes d'étalonnages n'améliore pas la calibration du modèle. Elles augmentent même l'ECE, cela peut être du au fait que l'on voit que le modèle est très sous-confiant à un moment or ces méthodes semble être efficaces uniquement pour améliorer la calibration des modèles qui sont très sur-confiants.
...
Le modèle non calibré a déjà une confiance moyenne proche des performances réelles du modèle, ce qui semble rendre inefficaces les deux méthodes d'étalonnage.
