...
$$
où $z_k$ correspond pour tous $k$ appartenant à $\llbracket 1, K \rrbracket$ au score associé a la classe k. On pose :
$$
...
On définit $\lambda=(\lambda_1,...,\lambda_K)$ où $\lambda \in \Lambda_{K}$ qui est donc par définition une mesure de probabilité sur $\{1, \ldots, K\}$ et $\pi=(\frac{1}{K},...,\frac{1}{K})\in \mathbb{R}^{K}$ ($\pi$ suit une distibution uniforme sur $\{1, \ldots, K\}$). La divergence de Kullback-Leibler entre $\lambda$ et $\pi$ est alors définie de cette manière :
$$
...
$$
\left(\sigma_1,\dots, \sigma_K \right)=\arg\min_{\lambda \in \Lambda_{K}}\left\{ -\sum_{j=1}^{K}\lambda_jz_{j}+T.K(\lambda,\pi) \right\}
$$
On trouve donc que quand $T \to 0^+$, 
$$
\left(\sigma_1,\dots, \sigma_K \right)\to \arg\max_{\lambda \in \Lambda_{K}}\left\{ \sum_{j=1}^{K}\lambda_jz_{j}\right\}
$$ 
donc $\left(\sigma_1,\dots, \sigma_K \right)\to \delta_{z_{max}}$. De plus, quand $T \to \infty$, 

$$
\left(\sigma_1,\dots, \sigma_K \right)\to\arg\min_{\lambda \in \Lambda_{K}}\left\{K(\lambda,\pi) \right\}
$$
