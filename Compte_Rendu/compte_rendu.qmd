...
::: {layout-ncol=2}
![Modèle non calibré](Figure_1.svg){width=230 #fig-1 fig-pos='H'}

![Modèle calibré avec TS](Figure_2.svg){width=230 #fig-2 fig-pos='H'}
:::
![Modèle calibré avec TEC](Figure_3.svg){width=230 #fig-3 fig-pos='H'}


En comparant @fig-1 avec @fig-2 et @fig-3 on remarque que c'est bien le cas. Pour interpréter ces Figures, on voit que sur @fig-1 le modèle a tendence a etre sur-confiant, car les probabilités qu'il prédit (confidence) sont supèrieurs à l'Expected accuracy qui est la précision rélle du modèle. Un modèle parfaitement calibré reste sur la diagonale, les barres rouges montre l'écart entre la confiance et la précision du modèle. Pour la @fig-1 on voit que la confiance moyenne est largement supèrieir à la précision moyenne du modèle ce qui montre clairement que le modèle est sur-confiant. Tandis qu'on voit que sur les @fig-2 et @fig-3  le modèle est mieux calibré et il donne des prédictions plus cohérentes au vu de la précision du modèle.
