...
D'après les résultats obtenus dans @tbl-j  on voit que les deux méthodes n'améliorent pas le score Brier et L'Expected Calibration Error et donnent des résultats moins bons que le modèle non calibré, cela peut être du au fait que le modèle a une erreur de classification trés faible et au fait que les deux tempèratures sont infèrieurs à 1.


Pour le modèle CIFAR-100 Resnet44 les valeurs trouvées pour température TS et température $T_{EC}$ sont respectivement 1.71 et 1.76 de plus l'erreur de classification vaut 0.28.
...
::: {#fig-ECE layout-ncol=3 fig-pos='H'}
![Modèle non calibré](Figure_nc_c10.svg){width=170 #fig-4}

![Modèle calibré avec TS](Figure_ts_c10.svg){width=170 #fig-5}

![Modèle calibré avec TEC](Figure_ec_c10.svg){width=170 #fig-6}
...
En comparant @fig-4 avec @fig-5 et @fig-6 on voit que pour ce modèle, les deux méthodes d'étalonnages n'améliore pas la calibration du modèle. Elles augmentent même l'ECE, cela peut être du au fait que l'on voit que le modèle est trés sous-confiant à un moment or ces méthodes semble être efficaces uniquement pour améliorer la calibration des modèles qui sont trés sur-confiants.
...
= \frac{1}{n}\sum_{k=1}^{m}n_k[\ \frac{1}{n_k}\sum_{j=1}^{n_k}(f_{kj}-o_{kj})^2]\ 
$$
...
$$
\frac{1}{n_k}\sum_{j=1}^{n_k}(f_{kj}-o_{kj})^2 = \frac{1}{n_k}\sum_{j=1}^{n_k}(f_{kj}-o_{kj} - \bar{f}_k + \bar{o}_k)^2 
$$

Ainsi on peut réécrire le Brier Score comme : 

$$
B_s = \frac{1}{n}\sum_{k=1}^{m}n_k \left[ (\bar{f}_k - \bar{o}_k)^2 + \frac{1}{n_k}\sum_{j=1}^{n_k}(f_{kj}-o_{kj} - \bar{f}_k + \bar{o}_k)^2 \right] = \frac{1}{n} \sum_{k=1}^{m} \left[ (\bar{f}_k - \bar{o}_k)^2 + \frac{1}{n_k} \sum_{j=1}^{n_k} ((f_{kj} - \bar{f}_k) - (o_{kj} - \bar{o}_k))^2 \right]
$$

En développant le carré on a : 

$$ 
B_s = \frac{1}{n} \sum_{k=1}^{m} n_k \left[ (\bar{f}_k - \bar{o}_k)^2 + \frac{1}{n_k}\sum_{j=1}^{n_k}(o_{kj} - \bar{o}_k)^2 + \frac{1}{n_k}\sum_{j=1}^{n_k}(f_{kj} - \bar{f}_k)^2 - \frac{2}{n_k}\sum_{j=1}^{n_k} (f_{kj} - \bar{f}_k)(o_{kj} - \bar{o}_k) \right]
$$
...
c'est le terme de fiabilité des prévisions, il compare les probabilités prévues aux probabilités d'observations il peut être réduit en un bon qualibrage des prévisions. 

Le deuxième terme de cette somme est défini par $UNC - RES$ (et obtenu en faisant plus ou moins  ) :

$$
\frac{1}{n}\sum_{k=1}^{m}\sum_{j=1}^{n_k}n_k(o_{kj} - \bar{o}_k)^2 = \frac{1}{n}\sum_{k=1}^{m}\sum_{j=1}^{n_k}n_k(o_{kj}-\bar{o})^2 - \frac{1}{n}\sum_{k=1}^{m}n_{k}(o_k - \bar{o})^2 
$$
...
$$
B_s = \frac{1}{n}\sum_{k=1}^{m}n_{k}(\bar{f}_k - \bar{o}_k)^2 - \frac{1}{n}\sum_{k=1}^{m}n_{k}(\bar{o}_k - \bar{o})^2 + \bar{o}(1-\bar{o}) + \frac{1}{n}\sum_{k=1}^{m}\sum_{j=1}^{n_k}(f_k - \bar{f}_k)^2 - \frac{2}{n}\sum_{k=1}^{m}\sum_{j=1}^{n_k}(f_{kj} - \bar{f}_k)(o_{kj}- \bar{o}_k) 
$$
...
- $I$ : terme d'incertitude qui prend en compte la dispersion des observations. Il est défini par : 
$$
\bar{o}(1-\bar{o}) 
$$
avec $\bar{o} = \frac{1}{n}\sum_{i=1}^{n}o_i$
où $n$ représente le nombre total de prévisions émises et $n$
...
Par exemple si on regroupe tous les cas où 80% de probabilité de pluie était prévue, si il a plu 4 jours sur 5 on obtient ainsi une fiabilité parfaite. Il est défini par : 
$$ 
\frac{1}{N}\sum_{i=1}^{K}n_i(f_i - \bar{o})^2
$$

- $R$ : terme de résolution qui mesure la distance entre les probabilités d'occurence. Il est défini par : 

$$
\frac{1}{N}\sum_{i=1}^{K}n_i(\bar{o}_i - \bar{o})^2
$$

où $\bar{o}_i$ est la fréquence observée compte tenu des prévisions de probabilités $f_i$ et $K$ le nombre de prévisions uniques émises. 
...
Elle permet d'obtenir des performances d'étalonnage trés similaires à celles de la mise à l'échelle de la température (TS). 
D'après nos observations, pour les modèles Cifar100-resnet20 et Cifar100-resnet44, elle permet de presque parfaitement restaurer l'étalonnage du réseau.

Cependant, pour le modèle Cifar10-resnet20, les deux méthodes ne sont pas efficaces, cela semble être dû au fait que l'erreur de classification de ce modèle est très faible. 
Le modèle non calibré a déjà une confiance moyenne proche des performances réelles du modèle, ce qui semble rendre inéfficaces les deux méthodes d'étalonnage.

Pour conclure, si vous avez un modèle entraîné qui est en moyenne beaucoup plus confiant que les performances réelles du modèle, vous pouvez procéder à la méthode d'étalonnage EC et vous obtiendrez des résultats très similaires à la méthode de mise à l'échelle de la température (TS).
En revanche, si le modèle entraîné a une confiance moyenne dans ses prédictions proche de ses performances réelles, alors les deux méthodes d'étalonnage risquent de ne pas être efficaces.
